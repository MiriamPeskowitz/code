module Players
  class Computer < Player
    def move(board)
    #look in book for tips on winning. 
      if !board.taken?(5)
        move ="5"
      ...
      elsif board.turn_count == 3 && (board.postion(1) == board.postion(9)) || (board.position(3) == board.position(7))
        move = "2"

        #ask coaches for this. My brain just doesn't want to take it in. 
...
#  			 # I don't know where to begin, so I looked at the solution code as a place to start. That's what these next lines are. 

#  			 #http://neverstopbuilding.com/minimax and also alpha-beta pruning 
